...
| **`modify_reservation`** (party, time, pre-order, guest, status) | **`PATCH /api/reservations/amend`** with JSON **`confirmation_code`** (or `code` / `next_reservation_code`) **or** numeric **`id` / `reservation_id`** in the body from **GET /lookup**. If the tool sends JSON **null placeholders** for code/id, use query **`?id=`** or **`?reservation_id=`** with the same numeric id as lookup/`…/status` (e.g. `PATCH /api/reservations/amend?id=11`). Body still carries `preorder`, `party_size`, `starts_at`, etc. Same patch body on **`PATCH /by-code/{code}`** and **`PATCH /{id}`** (no `/status`). |
